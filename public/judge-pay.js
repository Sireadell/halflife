...
// node_modules/@x402/core/dist/esm/chunk-UF6R7D6H.mjs
...
function isAtomicAmount(amount) {
  return /^\d+$/.test(amount);
}
function findSpendControlAssetEntry(controls, requirement, defaultAsset) {
...
function resolveAtomicSpendCap(controls, requirement, defaultAsset) {
  if (controls === false) {
    return void 0;
  }
...
var x402Client = class _x402Client {
  ...
  applySpendControls(x402Version2, requirements, clientSchemesByNetwork) {
    ...
    const findAssetEntry = (requirement) => findSpendControlAssetEntry(controls, requirement, defaultAssetFor(requirement));
...
var setButtonBusy = (id, busy, text) => {
  const node = byId(id);
  if (!node) return;
  if (node.dataset.idleHtml === void 0) node.dataset.idleHtml = node.innerHTML;
  node.disabled = Boolean(busy);
  node.innerHTML = text !== void 0 ? esc(text) : node.dataset.idleHtml;
};
var setButtonReady = (id, text) => {
  const node = byId(id);
  if (!node) return;
  if (node.dataset.idleHtml === void 0) node.dataset.idleHtml = node.innerHTML;
  node.disabled = false;
...
function renderRunResult(status, body) {
  ...
  if (status >= 200 && status < 300 && verdict) {
    ...
    sentence = revoked ? `This agent's certificate was just revoked: ${body.reason || "its behavior no longer holds up."}` : arcWritten ? "The agent held up. Certificate issued and written to Arc." : body?.reason || "The check completed. No new Arc record was needed.";
  }
